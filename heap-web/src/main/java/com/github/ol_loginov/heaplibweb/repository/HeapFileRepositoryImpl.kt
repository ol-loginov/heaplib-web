...
class HeapFileRepositoryImpl(
    ...
    override fun persist(entity: HeapFile): HeapFile {
        ...
        jdbc
            .sql("insert into HeapFile(path, status, loadStart, loadFinish, loadProgress, loadMessage, loadError)  values(:path, :status,  :loadStart, :loadFinish,  :loadProgress,  :loadMessage, :loadError)")
    ...
    override fun findById(entityId: Int): HeapFile? = jdbc
        .sql("select id, path, status, loadStart, loadFinish, loadProgress, loadMessage, loadError from HeapFile where id = :id")
        .param("id", entityId)
        .query(HeapFile::class.java)
        .optional().orElse(null)

    override fun findAllByOrderByLoadStartDesc(): List<HeapFile> = jdbc
        .sql("select id, path, status, loadStart, loadFinish, loadProgress, loadMessage, loadError from HeapFile order by loadStart desc")
        .query(HeapFile::class.java)
        .list()

    override fun findAllByStatusIn(statuses: List<HeapFileStatus>): List<HeapFile> = jdbc
        .sql("select id, path, status, loadStart, loadFinish, loadProgress, loadMessage, loadError from HeapFile where (:statusesLen = 0 or status in (:statuses)) order by loadStart desc")
    ...
    override fun findAllByStatusNotIn(statuses: List<HeapFileStatus>): List<HeapFile> = jdbc
        .sql("select id, path, status, loadStart, loadFinish, loadProgress, loadMessage, loadError from HeapFile where (:statusesLen = 0 or status not in (:statuses)) order by loadStart desc")
        .param("statusesLen", statuses.size)
        .param("statuses", statuses)
        .query(HeapFile::class.java)
        .list()
    ...
    override fun findFirstByPathOrderByIdDesc(path: String): HeapFile? = jdbc
        .sql("select id, path, status, loadStart, loadFinish, loadProgress, loadMessage, loadError from HeapFile where path = :path order by id desc limit 1")
        .param("path", path)
        .query(HeapFile::class.java)
        .optional().orElse(null)
}