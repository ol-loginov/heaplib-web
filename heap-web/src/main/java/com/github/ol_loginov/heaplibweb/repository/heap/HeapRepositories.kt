...
class HeapRepositories(
     ...
     */
    private val insertByLoadLocalFile: Boolean
        get() = jdbc.sql("select @@local_infile").query().singleValue() == "1"
    ...
    private fun executeTablesScript(scriptResource: String) {
        val script = HeapRepositories::class.java.getResourceAsStream(scriptResource).use {
            if (it == null)
